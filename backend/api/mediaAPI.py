...
_ROOT = os.path.realpath(MAIN_MEDIA_PATH) if MAIN_MEDIA_PATH else ""
...
def get_media(request: Request, path: str):
    ...
    # realpath (not abspath) resolves symlinks, so a symlink planted inside
    # the media root can't be used to escape it.
    full_path = os.path.realpath(os.path.join(_ROOT, path))
