...
def create_video_player_view(video_id):
    ...
    return html.Div([
        ...
        dcc.Store(id='video-seek-store', data=None),  # NEW: Store for video seeking
    ])
...
def handle_chat(n_clicks, query, video_id):
    ...
    try:
        ...
        response_div = html.Div([
            ...
            html.P(
                f"⏱️ Timestamp: {start_fmt} - {end_fmt}",
                className='text-muted',
                style={'fontSize': '14px'}
            ),
        ])
        
        # Return the response, clear input, and timestamp for video seeking
...
clientside_callback(
    ...
    function(timestamp) {
        if (timestamp !== null && timestamp !== undefined) {
            ...
            if (video) {
                ...
                // Optional: Uncomment the line below to auto-play after seeking
                // video.play();
            }
        }
    ...
    """,
    Output('video-player', 'id'),  # Dummy output
